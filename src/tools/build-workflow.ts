...
export type BuildWorkflowDeps = {
  ...
  mcp: {
    ...
    getNodeTypes(nodeTypes: string[]): Promise<string>
  }
  now?: () => Date
}
...
export async function buildWorkflow(deps: BuildWorkflowDeps): Promise<BuildWorkflowResult> {
  ...
  const nodeTypeDocumentation = await deps.mcp.getNodeTypes(nodeTypes)
  const plan = await deps.planner.createPlan({
    prompt: deps.args.prompt,
    sdkReference,
    nodeDocumentation: [{ nodeType: "selected", documentation: nodeTypeDocumentation }],
  })
...
function extractNodeTypes(searchResult: string): string[] {
  return [
    ...new Set(searchResult.match(/\bn8n-nodes-[a-z0-9_-]+(?:\.[a-z0-9_-]+)+\b/gi) ?? []),
  ].slice(0, 20)
}
