...
当前状态：`v0.2` 版本。核心的“托管 workflow”生命周期保持 v0.1 的保守安全边界，并新增可选的真实 n8n Docker E2E 验证层，用于在本地 n8n 实例中验证 API、MCP 和插件 smoke 路径。
...
v0.2 增加了一个显式触发的 E2E 验证层，用本地 Docker n8n 实例验证插件的核心生命周期。默认的 `npm test` 不会启动 Docker，也不要求真实 n8n。
...
该命令会启动 `docker-compose.e2e.yml` 中定义的 n8n，等待 n8n 可访问，将 `N8N_E2E_BASE_URL`、`N8N_E2E_MCP_URL`、`N8N_E2E_API_KEY` 和可选 MCP token 映射给测试进程，然后运行 `tests/e2e/**/*.e2e.test.ts`。
...
- 默认 MCP URL 是 `http://127.0.0.1:5678/mcp`。
...
- 真实 MCP endpoint smoke 检查；需要 Bearer auth 时支持 `N8N_E2E_MCP_TOKEN`。
- 插件 smoke E2E：通过插件工具路径连接真实测试配置。

v0.2.0 最近一次本地验证结果（不含 Docker E2E）：

- TypeScript：`./node_modules/.bin/tsc --noEmit` 通过。
- Vitest：`./node_modules/.bin/vitest run` 通过，15 个测试文件，143 个测试通过。
- 有 Docker 和本地测试 API Key 时再运行 `N8N_E2E_API_KEY=<你的测试 API Key> npm run test:e2e`。

## 当前版本状态

`0.2.0` 是真实 n8n E2E 验证里程碑：
