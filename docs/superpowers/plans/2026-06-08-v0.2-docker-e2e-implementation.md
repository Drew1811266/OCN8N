...
export function createE2eRuntimeConfig(input: E2eRuntimeConfigInput): PluginConfig {
  ...
  const mcpUrl = input.env.N8N_E2E_MCP_URL ?? `${baseUrl.replace(/\/api\/v\d+\/?$/i, "")}/mcp-server/http`
...
If this image tag is unavailable during implementation, replace it with a documented stable n8n Docker image tag that supports instance-level workflow-builder MCP tools, and record the exact tag in the README E2E section.
...
调试模式下，测试结束后 Docker n8n 会保持运行，方便进入 UI 查看 workflow 和日志。普通模式会停止容器并移除 orphan container，但默认保留 n8n Docker volume；只有设置 `N8N_E2E_REMOVE_VOLUMES=1` 时才会移除 volume。
```
